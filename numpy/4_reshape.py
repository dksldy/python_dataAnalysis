'''
    ...
    - 한축의 크기를 자동 계산하고자 할 경우 -1 사용 (단, -1은 한 번만 사용 가능)
'''
import numpy as np
...
def reshape_1d_to_2d():
  ...
  # arr.reshape(-1,-1)  # => -1을 한번만 작성 (ValueError)
# reshape_1d_to_2d()
...
def reshape_2d_to_3d():
  ...
  # arr_3d = arr.reshape(3, -1, -1) # => 다차원에서도 -1 은 한번만 가능 (ValueError)
# reshape_2d_to_3d()
...
def reshape_to_1d():
  ...
  arr_ravle = arr_3d.ravel()
  print('lavel : ',arr_ravle)
# reshape_to_1d()
...
def copy_and_view():
  ...
  print(f'flatten array : {flat_arr}')
  print(f'origin array : {origin_arr}')
  # => 원본 배열의 데이터 변경 없음. flatten -> 새로운 배열을 반환
