...
def dtype_int():
  ...
  #     np.array(data, dtype = np.지정할_타입)
  arr = np.array(data, dtype=int)
  print_info(arr)   # 기본형으로 생성됨! (int64)

  arr = np.array(data, dtype=np.int32)
  print_info(arr)   # 지정한 타입(int32) 로 변환됨

  arr = np.array(data, dtype='i4')
  print_info(arr)   # 문자 형태로 타입 지정('i4' == np.int32)
...
def dtype_str():
  ...
  # 바이트 문자열 표현 시 b'---' 와 같이 b 문자 추가
  print_info(arr)
  # |S1 : 바이트 문자열, 최대길이 1

  arr = np.array(['a', 'b', 'c'], dtype='S5')
  print_info(arr)
# dtype_str()
...
def dtype_bool():
  ...
  arr = np.array([-1, 0, 5.2], dtype=np.bool_)
  print_info(arr)
  # 0 -> False, 양/음수 -> True, 실수 -> True
