...
def set_seed():
  '''
      시드 설정 ? 
        시드를 고정하면 이후에 코드를 실행해도 동일한 난수가 생성됨 
          => 재현성 확보를 위함
  '''
  np.random.seed(42)
# set_seed()
...
def random_int():
  ...
  print(f'2차원 배열 랜덤 생성 :\n{random_2d}')

  # 3차원 배열 생성
  random_3d = np.random.randint(1, 101, size=(3, 2, 2))
  print(f'3차원 배열 랜덤 생성 :\n{random_3d}')
# random_int()

def normal_dist():
  '''
      정규 분포 (Normal Distribution) - 종 모양, 코끼리 먹은 보아뱀
      : 자연 현상(키, 몸무게, 시험 점수 등)에서 가장 흔하게 나타나는 분포
      : 평균을 중심으로 데이터가 모이고, 양 끝으로 갈수록 희박해지는 모양의 그래프를 띔

      표준편차 (Standard Deviation)
      : 데이터가 평균으로부터 얼마나 떨어져 있는지(퍼져있는지)를 나타내는 값
      : 표준 편차가 작을 수록 데이터는 평균 주변에 좁게 모여있음
