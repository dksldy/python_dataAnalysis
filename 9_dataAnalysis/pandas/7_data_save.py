...
df_re_pro = pd.pivot_table(df
               , index='Region'
               , values='Profit'
               , aggfunc='sum')
print(f'\npivot_table 활용 Region 별 Profit 합계\n{df_re_pro}')
...
# * Excel 저장
#     to_excel(파일명, index=인덱스행 저장여부, sheet_name=시트명)
#     - sheet_name : 저장될 시트 이름. 기본값 : Sheet1
#     => 내부적으로 이미 유니코드 기반으로 문자를 저장하도록 설계되어 있음
save_excel_filepath = 'result.xlsx'
df_summary.to_excel(save_excel_filepath, index=False, sheet_name='Summaary')
print(f'\n*** 엑셀파일로 저장 완료 : {save_excel_filepath} ***')
